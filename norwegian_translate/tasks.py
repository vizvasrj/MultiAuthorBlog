...
from norwegian_translate.models import NorwegianTranslatedPost
...
def text_to_speech(text, pk, part=None):
    ...
    if part != None:
        ...
        with open(f'{pk}_{part}_norwegian.mp3', 'wb') as output:
            output.write(response.audio_content) 
        
    if part == None:
        with open(f'{pk}_norwegian.mp3', 'wb') as output:
            output.write(response.audio_content) 
        

@shared_task
def norwegian_translate(pk):
    ...
    # before uploading to norwegian_tra.. replace it with pre
    t_t_p = t_t
    ...
    # saving to translate models ie Norwegian
    try:
        if parts != None:
            audio = []
            for x in range(len(parts)):
                audio.append(f"AudioSegment.from_mp3(f'{pk}_{x}_norwegian.mp3')")
            join = "+".join(audio)
            "mix_audio = " + join
            executes=("mix_audio = " + join)
            executes2=(f"mix_audio.export('{pk}_norwegian.mp3')")
            exec(executes)
            exec(executes2)
            
            # mix_audio.export("{pk}_norwegian.mp3")
            rm_audio = []
            for x in range(len(parts)):
                import os
                os.system(f'rm {pk}_{x}_norwegian.mp3')
    except:
        pass
    file = open(f'{pk}_norwegian.mp3', 'rb')
    fileU = File(file)
    
    h = NorwegianTranslatedPost.objects.create(
        post_id=pk,
        title=str(t_p_title),
        # title=post.title,
        body=str(t_t_p_s),
    ...
    import os
    os.system(f'rm {pk}_norwegian.mp3')
    for t_s in t_k_l:
        h.tags.add(t_s)
