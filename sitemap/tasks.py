...
# from sitemap.tasks_google import sitemap_add_google
...
def basethousands(url, date):
    whole = "https://vizvasrj.com/" + url
    a = (
        '<url xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><loc>'
        + whole
        + "</loc><lastmod>"
        + date
...
def get_base_one(start_bracket, end_bracket, abs_url, date):
    from .models import Sitemap

    urlset = start_bracket + "".join(abs_url) + end_bracket
    with open(f'base_sitemap_{date}.xml', 'w') as f:
        f.write(urlset)
    sfile = File(open(f'base_sitemap_{date}.xml', 'r'))
    os.remove(f'base_sitemap_{date}.xml')
    obj = Sitemap.objects.create(
        name=f'base_sitemap_{date}.xml',
        sitemap_file=sfile
    )
    # print(obj)
    ...
    # sitemap_add_google(sitemap_url=full_url)
...
def sitemap_creator():
    ...
    sdata = []
    abs_url = []
    idx = timezone.now().strftime("%d-%m-%Y")
    for post in Post.objects.filter(updated__gt=timezone.now() - timezone.timedelta(1)):
        ...
        # base one
        base_sitemap_url = basethousands(jurl, date)
        abs_url.append(base_sitemap_url)
        data = {
            'url': sitemap_url
        }
        sdata.append(data)
    ...
    # this function witout en, hi, de url in it
    get_base_one(start_bracket=start_bracket, end_bracket=end_bracket, abs_url=abs_url, date=idx)
    ar = []
    zh_hans = []
    en = []
    ta = []
    ...
    ar_sitemap = chunks(ar_url, 49000)
    zh_hans_sitemap = chunks(zh_hans_url, 49000)
    en_sitemap = chunks(en_url, 49000)
    ta_sitemap = chunks(ta_url, 49000)
    fr_sitemap = chunks(fr_url, 49000)
    de_sitemap = chunks(de_url, 49000)
    hi_sitemap = chunks(hi_url, 49000)
    id_sitemap = chunks(id_url, 49000)
    it_sitemap = chunks(it_url, 49000)
    ja_sitemap = chunks(ja_url, 49000)
    ko_sitemap = chunks(ko_url, 49000)
    nn_sitemap = chunks(nn_url, 49000)
    pt_sitemap = chunks(pt_url, 49000)
    ru_sitemap = chunks(ru_url, 49000)
    es_sitemap = chunks(es_url, 49000)
    vi_sitemap = chunks(vi_url, 49000)


    for x in ar_sitemap:
        with open(f'ar_sitemap_{idx}.xml', 'w') as f:
    ...
    for x in zh_hans_sitemap:
        with open(f'zh_hans_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in en_sitemap:
        with open(f'en_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in ta_sitemap:
        with open(f'ta_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in fr_sitemap:
        with open(f'fr_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in de_sitemap:
        with open(f'de_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in hi_sitemap:
        with open(f'hi_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in id_sitemap:
        with open(f'id_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in it_sitemap:
        with open(f'it_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in ja_sitemap:
        with open(f'ja_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in ko_sitemap:
        with open(f'ko_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in nn_sitemap:
        with open(f'nn_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in pt_sitemap:
        with open(f'pt_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in ru_sitemap:
        with open(f'ru_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in es_sitemap:
        with open(f'es_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
    ...
    for x in vi_sitemap:
        with open(f'vi_sitemap_{idx}.xml', 'w') as f:
            one = start_bracket + "".join(x) + end_bracket            
            f.write(one)
