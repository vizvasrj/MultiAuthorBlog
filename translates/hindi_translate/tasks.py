...
def text_to_speech(text, pk, part=None):
    ...
    if part != None:
        ...
        with open(f'{pk}_{part}_hindi.mp3', 'wb') as output:
            output.write(response.audio_content) 
        
    if part == None:
        with open(f'{pk}_hindi.mp3', 'wb') as output:
            output.write(response.audio_content) 
        

@shared_task
def hindi_translate(pk):
    ...
    if len(t_t_n_s_t) > 5000:
        ...
        for x in range(len(parts)):
            print(len("।".join(parts[x])),"len("".join(parts[x]))", x)
        for x in range(len(parts)):
    ...
    try:
        if parts != None:
            audio = []
            for x in range(len(parts)):
                audio.append(f"AudioSegment.from_mp3(f'{pk}_{x}_hindi.mp3')")
            join = "+".join(audio)
            "mix_audio = " + join
            executes=("mix_audio = " + join)
            executes2=(f"mix_audio.export('{pk}_hindi.mp3')")
            exec(executes)
            exec(executes2)
            
            # mix_audio.export("{pk}_hindi.mp3")
            rm_audio = []
            for x in range(len(parts)):
                import os
                os.system(f'rm {pk}_{x}_hindi.mp3')
    except:
        pass

    file = open(f'{pk}_hindi.mp3', 'rb')
    fileU = File(file)
